...
import com.monksoft.sports.databinding.ActivityMainBinding
import com.monksoft.sports.pojos.Sport
import com.monksoft.sports.retrofit.WeatherEntity
import com.monksoft.sports.retrofit.WeatherService
...
//            }
    ...
    private suspend fun getWeather(): WeatherEntity = withContext(Dispatchers.IO) {
        ...
        service.getWeatherById(3530597L, "metric", "2e7415bc983752b966ca6c3ef0b9bddb")
    }
        ...
        binding.etNumber.addTextChangedListener {
            ...
            fibScope = CoroutineScope(Job() + exceptionHandler).launch (Dispatchers.Default) {
                if(it.toString().isNotEmpty()){
                    val fib = fibonaccii(it.toString().toLong())
                    withContext(Dispatchers.Main){
                        binding.tvResult.text = "R = ${"%,d".format(fib)} in ${System.currentTimeMillis() - time}ms"
                    }
                }
            }
        }
    ...
    private fun fibonaccii(n: Long): Long {
        return if(n<=1) n
        else fibonaccii(n-1) + fibonaccii(n-2)
    }
